...
Each machine needs to be configured to **exactly the specified joltage levels** to function properly. Below the buttons on each machine is a big lever the you can use to switch the buttons from configuring the indicator lights to increasing the joltage levels. (Ignore the indicator light diagrams).
